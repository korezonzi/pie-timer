...
```
src-tauri/target/release/bundle/macos/Pie Timer.app   # アプリ本体
src-tauri/target/release/bundle/dmg/Pie Timer_0.1.0_x64.dmg
```

`/Applications` へ配置すると Raycast や Finder から検索・起動できます:

```bash
ditto "src-tauri/target/release/bundle/macos/Pie Timer.app" "/Applications/Pie Timer.app"
```

> グローバルショートカットを使うには、macOS の **システム設定 → プライバシーとセキュリティ → アクセシビリティ** で Pie Timer を許可してください。
...
| 症状 | 対処 |
|------|------|
| グローバルショートカットが効かない | システム設定のアクセシビリティで Pie Timer を許可 |
